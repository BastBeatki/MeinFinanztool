...
import { Trash2, ArrowUpRight, ArrowDownLeft, Search, Landmark, Coins, CheckSquare, Square, Edit2, X, Check, RefreshCw } from 'lucide-react';

interface TransactionListProps {
    transactions: Transaction[];
    onDelete: (id: string) => void;
    onUpdate: (tx: Transaction, updateRule?: boolean) => void;
}
...
const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete, onUpdate }) => {
    ...
    const saveEdit = (tx: Transaction) => {
        ...
        if (!isNaN(num) && editDate) {
            onUpdate({ ...tx, amount: num, date: editDate }, updateRule);
        }
        setEditingId(null);
    };
                    ...
                    {transactions.map((tx) => {
                        ...
                        return (
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {tx.isRecurring && (
                                                    <span className="text-[10px] bg-purple-500/20 text-purple-400 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                                ...
                                {isEditing && tx.isRecurring && (
                                    <div className="flex items-center gap-2 pt-2 border-t border-slate-700/50">
                                        <input 
                                            type="checkbox" 
                                            id={`update-rule-${tx.id}`}
                                            checked={updateRule}
                                            onChange={(e) => setUpdateRule(e.target.checked)}
                                            className="w-4 h-4 rounded border-slate-600 bg-slate-900 text-blue-500"
                                        />
                                        <label htmlFor={`update-rule-${tx.id}`} className="text-xs text-slate-400 select-none cursor-pointer">
                                            Auch künftige (Regel) anpassen?
                                        </label>
                                    </div>
                                )}
                            </div>
