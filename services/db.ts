...
import { Transaction, RecurringRule, PotConfig } from '../types';

const DB_NAME = 'FinanceFlowDB';
const DB_VERSION = 17; // Bumped version
const STORE_TRANSACTIONS = 'transactions';
const STORE_RULES = 'recurring_rules';
const STORE_POT_SETTINGS = 'pot_settings';
...
export class DBService {
    ...
    async init(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Pot Settings Store
                if (!db.objectStoreNames.contains(STORE_POT_SETTINGS)) {
                    db.createObjectStore(STORE_POT_SETTINGS, { keyPath: 'id' });
                }
            };
            ...
            request.onsuccess = async (event) => {
                ...
                // Migrations are kept but rely on data state rather than forced simulation date
    ...
    private async runMigrations(): Promise<void> {
        ...
        // 1. Fix Joyn & RTL PLUS for Nov 2025 (Legacy fix)
    ...
    private async seedInitialRules(): Promise<void> {
        ...
        // Use REAL DATE for new users
        const now = new Date(); 
        const currentMonthPrefix = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;

        // 1. Initial Balances (Example values, user can adjust via manual transaction if needed)
        // We only add this if NO transactions exist to avoid duplicates on re-seed attempt
        const txCount = await this.getAllTransactions();
        if (txCount.length === 0) {
            await this.addTransaction({
                ...
                type: 'income', 
                category: 'Startsaldo',
                note: 'Bitte Startsaldo anpassen',
                method: 'digital',
                account: 'bank',
                status: 'completed',
                isRecurring: false,
    ...
    async getPotConfigs(): Promise<PotConfig[]> {
        return new Promise((resolve, reject) => {
            if (!this.db) return reject("DB not initialized");
            const transaction = this.db.transaction([STORE_POT_SETTINGS], 'readonly');
            const store = transaction.objectStore(STORE_POT_SETTINGS);
            const request = store.getAll();
            request.onsuccess = () => resolve(request.result);
            request.onerror = () => reject(request.error);
        });
    ...
    async savePotConfig(config: PotConfig): Promise<void> {
        return new Promise((resolve, reject) => {
            if (!this.db) return reject("DB not initialized");
            const transaction = this.db.transaction([STORE_POT_SETTINGS], 'readwrite');
            const store = transaction.objectStore(STORE_POT_SETTINGS);
